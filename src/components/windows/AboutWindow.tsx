...
.skeleton-bar { animation: skeletonPulse 1.5s ease infinite; }
...
export default function AboutWindow() {
  ...
  const avatarSrc = profile?.avatar_url ?? '/assets/avatar.png'
  ...
  const stats = profile ? [
    ...
    { key: 'exp',    value: profile.experience },
    { key: 'stack',  value: profile.stack },
  ] : []

  const channels = profile?.open_to?.map((v, i) => ({
    key: `ch_${i + 1}`,
    value: v,
  })) ?? [
  ...
  return (
    <>
      ...
      <div className="about-panels" style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>

        {/* LEFT: AVATAR PANEL */}
        <div className="avatar-panel" style={{
          width: '40%', borderRight: '1px solid #00ff4633',
          display: 'flex', flexDirection: 'column', alignItems: 'center',
          justifyContent: 'flex-start', padding: '16px 12px', gap: 10, flexShrink: 0,
        }}>
          <div style={{
            position: 'relative', width: '100%', maxWidth: 140,
            animation: 'avatarGlitch 0.6s ease forwards',
            animationDelay: '0.2s', animationIterationCount: 1, opacity: 0,
          }}>
            {(!isLoaded || hasError) && (
              <div style={{
                width: '100%', aspectRatio: '3/4', background: '#00ff4608',
                border: '1px solid #00ff4633', borderRadius: 4,
                display: 'flex', flexDirection: 'column', alignItems: 'center',
                justifyContent: 'center', gap: 6,
              }}>
                <IconUser size={32} color="#00ff4633" />
                {hasError && <span style={{ fontSize: 9, color: '#00ff4633' }}>avatar not found</span>}
              </div>
            )}
            <img
              src={avatarSrc}
              ...
              style={{
                ...
                inset: 0, width: '100%', maxWidth: 140, aspectRatio: '3/4',
                objectFit: 'cover', objectPosition: 'center top',
                border: '1px solid #00ff4666', borderRadius: 4,
              }}
            />
            {isLoaded && !hasError && <>
              <div style={{ position: 'absolute', inset: 0, background: 'repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,255,70,0.04) 2px,rgba(0,255,70,0.04) 4px)', pointerEvents: 'none', borderRadius: 4 }} />
              <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,255,70,0.06)', mixBlendMode: 'screen', pointerEvents: 'none', borderRadius: 4 }} />
            </>}
          </div>

          <div style={{
            background: '#00ff4615', border: '1px solid #00ff4666', borderRadius: 2,
            padding: '3px 10px', fontSize: 10, color: '#00ff46', textAlign: 'center',
            letterSpacing: 1, display: 'flex', alignItems: 'center', gap: 5,
          }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#00ff46', display: 'inline-block', animation: 'pulse-glow 2s ease-in-out infinite', flexShrink: 0 }} />
            {profile?.available !== false ? 'ONLINE // AVAILABLE' : 'ONLINE // BUSY'}
          </div>

          <span style={{ fontSize: 11, color: '#00ff4699', textAlign: 'center', letterSpacing: 2, textTransform: 'uppercase' }}>
            {profile?.name ?? 'uday bhatnagar'}
          </span>
        </div>

        {/* RIGHT: TERMINAL STATS */}
        <div className="stats-panel" style={{
          width: '60%', padding: '14px 16px', overflowY: 'auto',
          display: 'flex', flexDirection: 'column', gap: 4,
        }}>
          <div style={{ color: '#00ff4644', fontSize: 11, marginBottom: 8 }}>// reading: about_me.txt</div>

          {loading ? <SkeletonLoader rows={6} /> : stats.map(s => (
          ...
          <div style={{ fontSize: 11, color: '#00ff4666', lineHeight: 1.8, fontStyle: 'italic', marginTop: 4 }}>
            {profile?.bio ? profile.bio.split('\n').map((l, i) => <span key={i}>{l}<br /></span>) : <>
              Engineer by training.<br />Artist by instinct.<br />
              Building at the intersection<br />of systems + creativity.
            </>}
          </div>
        </div>
      </div>
